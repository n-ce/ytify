...
const getEnglishSubs=(array,engSubIndex=0)=>{
	//preference-> english subtitles + not autogenerated , english subs+autogenerated ,first subtitles.
	
	for(let i=0;i<array.length;i++){
		if(array[i].code.includes("en") && array[i].autoGenerated===false)
			return array[i];
		if(array[i].code.includes("en") && array[i].autoGenerated===true)
			engSubIndex=i;
	}
	return array[engSubIndex];
}
	...
	fetch(api[instance] + '/streams/' + id)
		...
		.then(data => {
			...
			if (data.subtitles.length) {
				...
				const sub=getEnglishSubs(data.subtitles);
			
				audio.firstElementChild.src = sub.url;
			} else {
