...
export async function getList(
  ...
  try {
    const res = await fetch(
      `${store.api}/${type}?id=${id}${all ? "&all=true" : ""}`,
    );
    ...
    if (data.type === "artist") {
      ...
      setListStore({
        ...
        url: id,
        img: artist.img,
        list: (artist.items || []).map(
          (v) =>
            ({
              ...v,
              author: v.author.endsWith(" - Topic")
                ? v.author
                : `${v.author} - Topic`,
      ...
      setListStore({
        ...
        author:
          "author" in listData
            ? (listData as YTPlaylistItem | YTAlbumItem).author || ""
            : listData.name,
        type: listType,
        url: id,
        hasContinuation:
        ...
        list: (listData.items || []).map(
          (v) =>
            ({
              ...v,
              author:
                data.type === "album" && !v.author.endsWith(" - Topic")
                  ? `${v.author} - Topic`
                  : v.author,
